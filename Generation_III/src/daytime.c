...
char cDAYTIME_convertDstrToDstruct(//RET: 1=>DateOk  >0=>DateBad
...
long lDAYTIME_convertDstructToSec(	//RET: Seconds since Jan-1-2000
	...
	#if 0
	vSERIAL_rom_sout("FullDaysToDate= ");
	...
	#if 0
	vSERIAL_rom_sout("FullYears+DaysToDate= ");
	vSERIAL_IV32out(lVal);
	vSERIAL_crlf();
	#endif
	...
	#if 0
	vSERIAL_rom_sout("TotSecsToDate= ");
	vSERIAL_IV32out(lVal);
	vSERIAL_crlf();
	#endif
...
void vDAYTIME_convertDstructToShow( /* show date as 07/24/2004 */
	...
	if(ucTextOrNumericFlag == TEXT_FORM)
		vSERIAL_rom_sout(cpaMonthName[(spDate->ucMonth)-1]);
	else
	...
	vSERIAL_bout('/');
	if(ucTextOrNumericFlag == TEXT_FORM)  vSERIAL_rom_sout("20");	
...
char cDAYTIME_convertTstrToTstruct(	//RET: 1->TimeOk  >0->TimeBad
...
long lDAYTIME_convertTstructToSec(	//RET: converted HH:MM:SS to sec
	...
	#if 0
	vSERIAL_rom_sout("TotSecInHr+Min+Sec= ");
	vSERIAL_IV32out(lVal);
	vSERIAL_crlf();
	#endif
...
void vDAYTIME_convertDstructAndTstructToSetSysTime(
	...
	/* BUILD A REPORT MESSAGE THAT CLK HAS BEEN CHANGED */
	vREPORT_buildReportAndLogIt(
			SENSOR_TIMECHANGE_HI,			//Sensor 0
			(uint)(lSec/0x10000),			//Sensor 0 data
			SENSOR_TIMECHANGE_LO,			//Sensor 1
			(uint)lSec,						//Sensor 1 data
