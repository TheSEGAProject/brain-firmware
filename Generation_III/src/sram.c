...
//	unsigned char ucAddrLO;
	...
	/* TURN ON READ ENABLE */
	SRAM_SELRW_PORT |= SRAM_READWRITE;			//Turn on read

	/* SET BYTE REG TO INPUT */
	SRAM_DATA_DIR_PORT = 0x00;		  			//convert latch to an input

	/* TURN ON CHIP ENABLE */
	SRAM_SELRW_PORT |= SRAM_CHIP_SELECT;		//enable the data onto the bus

	/* READ THE DATA */
	ucDataByte = SRAM_DATA_IN_PORT;		  		//read the data

	/* TURN OFF CHIP ENABLE */
	SRAM_SELRW_PORT &= ~SRAM_CHIP_SELECT;  		//disable the data from the bus

	__bis_SR_register(GIE);						// Yes interrupts

	#if 0
	vSERIAL_rom_sout("SRM:RD[");
...
//	ucAddrHI = ((unsigned char) (uslAddr >>  8));
	...
	SRAM_DATA_OUT_PORT = ucDataByte;					//stuff data
   	...
   	/* FLOP THE CLK DOWN */
	SRAM_SELRW_PORT &= ~SRAM_CHIP_SELECT;				//set select back to deselect

	/* TURN OFF WRITE ENABLE */
	SRAM_SELRW_PORT |= SRAM_READWRITE;					//set Write ena back to read
