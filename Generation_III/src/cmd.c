...
static uchar ucCMD_chkSingleStr(
	...
	vSERIAL_rom_sout("CSS:ComparStrs: ");
	cpRamPtr = cpEnteredStr;
	while(*cpRamPtr != 0) vSERIAL_bout((uchar)*cpRamPtr++);
	vSERIAL_rom_sout(" to ");
	vSERIAL_rom_sout(cpSingleCmdStr);
	vSERIAL_crlf();
	}
	#endif
	...
	#if 0
	vSERIAL_rom_sout("CSS:MchVal= ");
	vSERIAL_UIV8out(ucMatchVal);
	vSERIAL_rom_sout(" = ");
	switch(ucMatchVal)
		{
		case 0:	vSERIAL_rom_sout("EStr<LStr"); break;
		case 1: vSERIAL_rom_sout("EStr==LStr"); break;
		case 2: vSERIAL_rom_sout("EStr><LStr:"); break;
		case 3: vSERIAL_rom_sout("EStr>LStr:"); break;
		default: vSERIAL_rom_sout("EStr??LStr:"); break;
		}
	vSERIAL_crlf();
	#endif
...
void vCMD_showCmdList(
		const char *cpaCmdStrArray[],	//ptr to cmd str (11 char max)
	...
	for(ucii=0;  ucii<ucCmdArraySize;  ucii++)
		{
		vSERIAL_rom_sout(cpaCmdStrArray[ucii]);
		vSERIAL_colTab(10);
