...
void vBUZ(
	...
	if(!ucMODOPT_readSingleRamOptionBit(OPTPAIR_BUZ_ON))
		{
		return;
		} 
	...
	TA0CTL_L = 0x90;					// (SMCLK_RATE/6)/4 = SMCLK_RATE/24

	/* SET TA0 MODE TO COMPARE, RESET/SET MODE*/
	TA0CCTL3 = 0x00E0;

	/* WAIT THE DURATION */
	vDELAY_wait100usTic(uiDuration_in_100us);
