...
#include "fram.h"			//Level 1 Ferro Ram Routines
...
#define SD_BD_IS_ATTCHD_YES						0x04	// 0000 0100
#define SD_BD_IS_ATTCHD_NO 						0x00	// 0000 0000
#define SD_BD_IS_ATTCHD_MSK						0x04	// 0000 0100
													
#define CHK_FOR_SD_ON_STUP_YES					0x08	// 0000 1000
#define CHK_FOR_SD_ON_STUP_NO					0x00	// 0000 0000
#define CHK_FOR_SD_ON_STUP_MSK					0x08	// 0000 1000
...
const char *cpaRoleName[NUMBER_OF_ROLES] =
{
	"None",				//---
	"SendOnly",			//--x
	"Hub",				//-r-
...
const uchar ucaRoleOptionDefaults[NUMBER_OF_ROLES][OPTION_BYTE_COUNT] =
{
...

  {
	...
	SD_BD_IS_ATTCHD_NO|				//2: SD board is attached
	CHK_FOR_SD_ON_STUP_NO|			//3: Check for SD on startup
...

  {
	...
	ALLOW_BRAIN_STUP_HBRNAT_YES|	//1: Allow Startup Brain Hibernation
	SD_BD_IS_ATTCHD_YES|  			//2: SD board is attached
	CHK_FOR_SD_ON_STUP_YES|			//3: Check for SD on startup
...
const char *cpaBitName[OPTION_BYTE_COUNT][8] =
{

	{						//OPTION BYTE 0

	"RdioSnd",				//0: Wisard sends radio msgs
	"RdioRec",				//1: Wisard receives radio msgs
	"SampDat",				//2: Wisard samples data from SD
	"",						//3: Wisard not implemented bit
	"Stream",				//4: Streaming (On/Off)
	"Systat",				//5: Show Systat (On/Off)
	"CkRdioBd",				//6: Check for Radio Board on startup (On/Off)
	"RdioRnd"				//7: Use Radio to Get Random Seed (On/Off)
	},

	{						//OPTION BYTE 1

	"RdioMsg",				//0: Send Radio Startup msg
	"BrHbrnt",				//1: Allow Startup Brain Hibernation
	"CkSdAtch",				//2: SD board is attached
	"CkSDStup",				//3: Check for SD on startup
	"FxSoil",				//4: Correct the soil readings
	"HbrnatCk",				//5: Do Hibernation Check on Startup
	"BuzOn",				//6: Buzzer on
	"CkForFrm"				//7: Check for Fram on Startup
	},

	{						//OPTION BYTE 2

	"CkFrmFmt",				//0: Check Fram format on Startup
	"CkForFsh",				//1: Check for Flash on Startup
	"ShwStsLf",				//2: Show Status on LFactors
	"ShowLnks",				//3: Show Link Schedules
	"",						//4: 
	"",						//5: 
	"",						//6: 
	""						//7: 
	...
	"RdioSLk",				//0: SOM2 LINKUP       report over radio
	"FlshSLk",				//1: SOM2 LINKUP	   report to flash
	"RdioFSLk",				//2: SOM2 FAILED LINK  report over radio
	"FlshFSLk",				//3: SOM2 FAILED LINK  report to flash
	"RdioRLk",				//4: ROM2 LINKUP	   report over radio
	"FlshRLk",				//5: ROM2 LINKUP	   report to flash
	"RdioFRLk",				//6: ROM2 FAILED LINK  report over radio
	"FlshFRLk"				//7: ROM2 FAILED LINK  report to flash
	},

	{						//OPTION BYTE 4

	"RdioRsts",				//0: SYS RESTART	   report over radio
	"FlshRsts",				//1: SYS RESTART	   report to flash
	"RdioBtDd",				//2: BATT DEAD		   report over radio
	"FlshBtDd",				//3: BATT DEAD		   report to flash
	"RdioBtV",				//4: BATT VOLTAGE 	   report over radio
	"FlshBtV",				//5: BATT VOLTATGE	   report to flash
	"RdioTime",				//6: TIME CHANGES	   report over radio
	"FlshTime"				//7: TIME CHANGES	   report to flash
	}
...
const char *cpaMODOPTcmdText[CMD_ARRAY_SIZE] = 
	{
	"Quit",									// 0 Quit
	"Help",									// 1 Help
	"showOptions",							// 2 Show Option bits
	"setOption",							// 3 Set an Option bit
	"clrOption",							// 4 Clr an Option bit
	"setRole"								// 5 Set a new role
...
void vMODOPT_showSingleRomOptionBit(
	...
	vSERIAL_rom_sout(" ");
	vSERIAL_padded_rom_sout(cpaBitName[ucByteIdx][ucBitIdx], 8);//show name
	vSERIAL_rom_sout(" ");

	cpStrPtr = "No";			//assume bit off
	if(ucaRoleOptionDefaults[ucOptionTblNum][ucByteIdx] & ucaBitMask[ucBitIdx])
		{
		cpStrPtr = "Yes";		//correct assumption
		}
	vSERIAL_padded_rom_sout(cpStrPtr,-3);	//show Bit state
...
void vMODOPT_showAllRomOptionTblsAllBits(
	...
	for(ucii=0;  ucii<NUMBER_OF_ROLES;  ucii++)
		...
		vSERIAL_rom_sout(" (\"");
		vSERIAL_rom_sout(cpaRoleName[ucii]);
		vSERIAL_rom_sout("\")\r\n");
...
uchar ucMODOPT_readFramOptionByte(
	...
	ucByteVal = ucFRAM_read_B8((uint)FRAM_OPTION_BYTE_0_ADDR + ucOptionByteIdx);
...
*
* NOTE: will not write if byte index if out of range
*
******************************************************************************/
...
void vMODOPT_writeFramOptionByte(
	...
	vFRAM_write_B8((uint)FRAM_OPTION_BYTE_0_ADDR + ucOptionByteIdx,
					ucByteVal
					);
...
void vMODOPT_showCurRole(
	...
	vSERIAL_rom_sout(cpaRoleName[ucRoleIdx]);
...
void vMODOPT_cmdModifyOptionTbls(
	...
	vNUMCMD_doCmdInputAndDispatch(
				'*',						//prompt
				cpaMODOPTcmdText,			//command array
...
void vMODOPT_cmdShowHelp(
	...
	{
	vNUMCMD_showCmdList(cpaMODOPTcmdText, CMD_ARRAY_SIZE);
	vSERIAL_crlf();
...
//	vSERIAL_dash(10);
	...
	vSERIAL_rom_sout("RoleID: ");
	cRoleIdx = cNUMCMD_getCmdNum(NUMBER_OF_ROLES);
	if(cRoleIdx <= 0)
		{
		vSERIAL_rom_sout("Abrt\r\n");
		goto setRoleExit;
		}
