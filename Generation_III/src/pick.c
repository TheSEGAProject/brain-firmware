...
#include "l2fram.h"			//Level 2 FRAM handlers
#include "pick.h"			//point pick reporting handler
#include "thermo.h"			//Thermocouple conversion routine
...
void vPICK_putSSP_tblEntry(
	...
	#if 0
	vSERIAL_rom_sout("E:pSSPte\r\n");
	vSERIAL_rom_sout("Tbl[");
	vSERIAL_UIV8out(ucSSP_TblNum);
	vSERIAL_rom_sout("]: ");
	vL2SRAM_showTblName((ulong)uslSSP_BaseAddr);
	vSERIAL_rom_sout("  SensorNum=");
...
ulong ulPICK_getSSP_tblEntry(
	...
	ulVal = ulL2SRAM_getGenericTblEntry(
...
void vPICK_initSSPtbls(
	...
	for(ucii=0; ucii<LAST_SSP_TBL_NUM_PLUS_1;  ucii++)
		{
		vSERIAL_rom_sout("SSP_tbl: ");
		vL2SRAM_showTblName(uslaSSP_BaseAddr[ucii]);
...
*		augemented to return the data form index which could be used to drive
...
void vPICK_showSSPtblHeader(
	...
	{
	vSERIAL_rom_sout("#### SENSOR  Y-PT TIME-POINT Y-DEL T-DEL\r\n");
	return;
